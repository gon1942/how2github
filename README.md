...
```
GITHUB에서는 ISSUE 항목에서 프로젝트의 모든 문제를 관리할 수 있도록 제공.
GITHUB에서 제공하는 이슈 템플릿을 활용하면 프로젝트의 버그, 관련된 기타 작업들의 문제를 제기할 때 정해진 규격의 ISSUE를 사용하게 되어 이슈 유형별로 명확하게 파악할 수 있게됨.

> 이슈 템플릿 작성.
*   GITHUB에서 제고하는 이슈 템플릿을 사용하거나, 별도의 이슈 템플릿을 작성하여 사용 가능.

> 이슈 처리 과정  
> ● 이슈 등록: 사용자 혹은 품질관리팀이 발견한 버그 혹은 신규 기능을 추가한다. 담당자가 명확한 경우 지정하기도 하나 공란으로 남겨두는 일도 방생함.  
> ● 이슈 검토/분류: 등록된 이슈는 개발팀이 검토. 중복, 재현 불가, 해결 불가능한 이슈의 경우 이 단계에서 이슈를 닫는다. 개발팀에서 해결할 이슈의 경우 담당자, 우선순위, 마감일 등을 지정.  
> ● 이슈 해결: 이슈가 해결되면 이슈를 Close. 담당자와 별개로 검증 담당자(Verifier)를 따로 두어 진짜로 이슈가 해결되었는지 검증하는 때도 있음.

>>> 이슈 템플릿을 사용하듯이 "PR에 대한 템플릿", "커밋 메시지 템플릿"등 다양한 종류의 템플릿을 제공하고 있으므로 필요한 부문에 대해 템플릿을 작성하여 사용하면 좋음. 
...
### 지속적 관리를 위해서...
```
Github Action은 github에서 제공하는 CI(지속적 통합)/CD(지속적 전달)를 제공하여 워크플로우를 자동화할 수 있게 도와주는 툴. 
...
> 
    ...
    Test Code 기반으로 Coverage 정보를 위처럼 자동으로 코멘드를 추가.
    누군가가 테스트 코드를 작성하지 않았다면 Change from base 항목에서 표시
    이렇게 해당 작업마다 커버리지를 표시하는 것이 전체 커버리지를 높이고 그 값을 유지하는 좋은 방법.
